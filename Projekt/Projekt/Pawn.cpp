...
void Pawn::createPossibleMoveTable()
{
	...
	if(coord.y + dy < 9 && coord.y + dy > 0 && costTable[coord.y -1 + dy][coord.x -1] < 2)	//works 100%	--moving forward
		possibleMoveTable[coord.y - 1 + dy][coord.x - 1] = true;
	
	if (coord.y + 2*dy < 9 && coord.y + 2*dy > 0 && costTable[coord.y - 1 + 2*dy][coord.x - 1] < 2)	//needs testing	--moving forward *2
